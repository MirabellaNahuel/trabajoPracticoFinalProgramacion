...
void cargaArchivoClientes(char nombreArchivo[],int id){
    ...
    if(clientes){
        do{
            ...
            encontrado=validacionClienteXEmail(clientes, cl.email);

            if(existe==1 || encontrado==1){
                if (existe==1 && encontrado==1){
                    printf("\nError - DNI y email duplicado\n");
                }
...
stClientes cambiaEmail(stClientes cl){
    ...
    do{
        ...
        fflush(stdin);
        gets(cl.email);
        existe=encuentraEmail(ARCHIVO_CLIENTES, cl.email);

        if(existe==1){
            printf("\nError - Email duplicado. Ingrese otro por favor.\n");
            getch();
        }
        system("cls");
...
/** \brief busca en un archivo un email
 *
 * \param recibe un archivo
 * \param recibe un email
 * \return retorna 1 si o encuentra y 0 si no lo encuentra
 *
 */

int encuentraEmail(char nombreArchivo[], char email[]){
    stClientes cl;
    int encontrado = 0;
    ...
    if(clientes){
        while(encontrado == 0 && fread(&cl, sizeof(stClientes), 1, clientes)>0){
          if(strcmp(cl.email, email)== 0){
              encontrado=1;
          }
...
/** \brief valida si un email ya se encuentra dentro del archivo
 *
 * \param recibe un puntero a un archivo
 * \param recibe un email
 * \return retorna 1 si esta en el archivo, y 0 si no esta
 *
 */

int validacionClienteXEmail(FILE *clientes, char email[]){
    int encontrado = 0;
    stClientes cl;
