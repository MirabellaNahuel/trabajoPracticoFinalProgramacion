...
/** \brief pasa los consumos en archivo de determinado cliente y mes a un arreglo
 *
 * \param recibe un archivo
...
int archi2arregloConsumosDeCliente(char nombreArchivo[], stConsumos c[], int dim, int idCliente, int mes){
    int v=0;
    stConsumos co;
...
/** \brief pasa los datos de consumo desde el archivo al arreglo
 *
 * \param nombreArchivo[] char
 * \param consumos[] stConsumos
 * \param dim int
 * \param v int
 * \param mes int
 * \return int v
 *
 */
int deArchivoAArregloConsumos(char nombreArchivo[], stConsumos consumos[], int dim, int v, int mes){
    stConsumos co;

    FILE *consu = fopen(nombreArchivo, "rb");

    if(consu){
        while(fread(&co, sizeof(stConsumos), 1, consu)>0 && v<dim){
            if(co.mes == mes){
            consumos[v]=co;
            v++;
            }
 ...
 *
 */
int buscaPosMenorFecha2(stConsumos c[], int v, int pos){
    int posMenor = pos;
    pos++;
...
/** \brief cambia el alta a baja en el archivo
 *
 * \param nombreArchivo[] char
 * \param idcliente int
 * \return void
 *
 */
void bajaDeConsumoXIdCliente(char nombreArchivo[],int idcliente){
    stConsumos co;
    ...
    if(consu){
        while(fread(&co, sizeof(stConsumos), 1, consu)>0){
            if(co.idCliente == idcliente && co.baja==0){
                fseek(consu, -1 * sizeof(stConsumos), SEEK_CUR);
                co.baja = 1;
                fwrite(&co, sizeof(stConsumos), 1, consu);
            }
...
/** \brief busca los consumos de un cliente por id en el archivo y lo muestra
 *
 * \param nombreArchivo[] char
 * \param id int
 * \return int flag
 *
 */
int consultaXIdConsumo(char nombreArchivo[], int id){
    stConsumos co;
    int flag=0;
